...
// composite functions with and without 2nd gaussian
...
void treeMacro() {
  ...
  double binN2 = 24;
   ...
   while (myReader.Next()) {    
     ...
     if(D0totPhi.GetSize() != 0) { //total d0-d0bars
       if (nChar[2] < 14.0) { //centrality constraint (0-80%)
	 for(double j = 0; j < D0totPhi.GetSize(); ++j) {
	   ...
	   if ((D0totPt[j] > 8 && D0totPt[j] < 16 && D0bartotPt[j] > 0.3) || (D0totPt[j] > 0.3 && D0bartotPt[j] > 8 && D0bartotPt[j] < 16)) {
	     ...
	     if (TMath::Abs(D0totEta[j])<0.9 && TMath::Abs(D0bartotEta[j])<0.9){ //pseudorapidity contraint
     ...
     if(pairCred0Phi.GetSize() != 0) { //pair creation d0-d0bars
       if (nChar[2] < 14.0) {
     ...
     if(glusplitd0barPhi.GetSize() != 0) { // gluon splitting d0-d0bars
       if (nChar[2] < 14.0) {
     ...
     if(flavexd0Phi.GetSize() != 0) { // flavour excitation d0-d0bars
       if (nChar[2] < 14.0) {
     ...
     if(ctotPt.GetSize() != 0) { // initial c-cbars
     ...
     if(D0totPt.GetSize() != 0 && D0totPhi.GetSize() != 0) {
       ...
       if (nChar[2] < 9.92) { //0-40% centrality
   ...
   //relative production mechanisms
